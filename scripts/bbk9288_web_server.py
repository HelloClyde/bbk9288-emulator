#!/usr/bin/env python3
"""Serve the BBK 9288 Web UI and manage its NAND workspace."""
...
class QemuController:
    def __init__(
        ...
        self.usb_connected = self._load_usb_connected()
    ...
    def _load_usb_connected(self) -> bool:
        try:
            payload = json.loads(self.settings_path.read_text(encoding="utf-8"))
            value = payload.get("usbConnected")
    ...
    def save_settings(self) -> None:
        ...
        try:
            with os.fdopen(fd, "w", encoding="utf-8") as staged:
                ...
                staged.write("\n")
            os.replace(staged_name, self.settings_path)
        finally:
            try:
...
class NandWorkspace:
    ...
    def update_settings(self, usb_connected: bool) -> dict:
        with self.lock:
            if self.active:
                raise ApiError(
                    "请先应用或放弃 NAND 更改",
            ...
            if self.qemu.usb_connected == usb_connected:
                return self.status()
            self.busy = "正在应用设备设置"
            was_running = self.qemu.running
            try:
                self.qemu.usb_connected = usb_connected
                self.qemu.save_settings()
                if was_running:
                    self.qemu.restart()
            finally:
                self.busy = None
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if parsed.path == "/api/settings":
                usb_connected = body.get("usbConnected")
                if not isinstance(usb_connected, bool):
                    raise ApiError("usbConnected 必须是布尔值")
                self._send_json(
                    self.workspace.update_settings(usb_connected)
                )
                return
...
def main() -> int:
    ...
    runtime_env = "BBK9288_RUNTIME_DIR"
    ...
    try:
        qemu.start()
        print(f"BBK 9288 Web server: http://127.0.0.1:{args.http_port}/")
        print(
