...
def populate_fat(
    ...
    target_component = target_name.strip("/")
    target_root = "/" + target_component if target_component else "/"
    files = 0
    total_bytes = 0
    fat = PyFatFS(
        str(flat_path),
    ...
    try:
        if replace_target:
            if target_root == "/":
                for name in fat.listdir("/"):
                    path = "/" + name
                    if fat.isdir(path):
                        fat.removetree(path)
                    else:
                        fat.remove(path)
            elif fat.exists(target_root):
                fat.removetree(target_root)
        if target_root != "/":
        ...
        for source in paths:
            ...
            target = (
                f"/{relative}" if target_root == "/"
                else f"{target_root}/{relative}"
            )
            if source.is_dir():
    ...
    verify_source = next(
        (
            ...
            if path.is_file() and path.name.upper() == "HZK_LIB.BIN"
        ),
        None,
    )
    ...
    if verify_source is None:
        raise ValueError(f"source tree contains no files: {source_path}")
    verify_relative = verify_source.relative_to(source_path).as_posix()
    verify_path = (
        f"/{verify_relative}" if target_root == "/"
        else f"{target_root}/{verify_relative}"
    )
    verify = PyFatFS(
        str(flat_path),
        encoding=encoding,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    install.add_argument(
        "--replace-target",
        action="store_true",
        help="remove the target directory contents before installing",
    )
    install.add_argument(
