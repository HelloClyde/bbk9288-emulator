...
void helper_s1c33_mac(CPUS1C33State *env, uint32_t rs)
{
    CPUState *cs = env_cpu(env);
    unsigned count_reg = rs & 0xf;
    unsigned lhs_reg = (count_reg + 1) & 0xf;
    unsigned rhs_reg = (count_reg + 2) & 0xf;
    uint32_t count = env->regs[count_reg];
    uint64_t accumulator = ((uint64_t)env->ahr << 32) | env->alr;
    ...
    while (count != 0) {
        ...
        if (cpu_memory_rw_debug(cs, env->regs[lhs_reg], lhs_buf,
                                sizeof(lhs_buf), false) != 0 ||
            cpu_memory_rw_debug(cs, env->regs[rhs_reg], rhs_buf,
                                sizeof(rhs_buf), false) != 0) {
            qemu_log_mask(LOG_GUEST_ERROR,
                          "s1c33: MAC memory read failed lhs=0x%08x "
                          "rhs=0x%08x remaining=%u\n",
                          env->regs[lhs_reg], env->regs[rhs_reg], count);
            break;
        }

        product = (int32_t)(int16_t)lduw_le_p(lhs_buf) *
                  (int32_t)(int16_t)lduw_le_p(rhs_buf);
        signed_sum = (__int128)(int64_t)accumulator + product;
        if (signed_sum > INT64_MAX || signed_sum < INT64_MIN) {
            env->psr |= S1C33_PSR_MO;
        }
        accumulator += product;
        env->regs[lhs_reg] += 2;
        env->regs[rhs_reg] += 2;
        count--;
        env->regs[count_reg] = count;
    }

    env->alr = accumulator;
    env->ahr = accumulator >> 32;
}
...
static bool s1c33_write_u32_debug(CPUS1C33State *env, uint32_t addr,
                                  uint32_t value)
{
    CPUState *cs = env_cpu(env);
    uint8_t buf[4];
...
G_NORETURN void helper_s1c33_brk(CPUS1C33State *env, uint32_t insn_pc)
{
    ...
    if (s1c33_read_u32_debug(env, 0x00000000, &handler) &&
        handler != 0 &&
        s1c33_write_u32_debug(env, 0x00000008, env->pc) &&
        s1c33_write_u32_debug(env, 0x0000000c, env->regs[0])) {
        env->debug_mode = true;
        env->psr &= ~S1C33_PSR_IE;
        env->pc = handler & ~1u;
        qemu_log_mask(CPU_LOG_INT,
                      "s1c33: BRK debug entry pc=0x%08x "
                      "handler=0x%08x saved-r0=0x%08x\n",
                      insn_pc, env->pc, env->regs[0]);
        cpu_loop_exit(cs);
    }

    warn_report("S1C33 guest shutdown: BRK without debug vector "
                "at pc=0x%08x next_pc=0x%08x", insn_pc, env->pc);
    s1c33_log_cpu_state(env);
    s1c33_enter_halt(env, cs, true, false);
}

G_NORETURN void helper_s1c33_retd(CPUS1C33State *env)
{
    CPUState *cs = env_cpu(env);
    uint32_t target;
    uint32_t r0;
    ...
    if (!env->debug_mode ||
        !s1c33_read_u32_debug(env, 0x00000008, &target) ||
        !s1c33_read_u32_debug(env, 0x0000000c, &r0)) {
        warn_report("S1C33 RETD outside a valid debugging context "
                    "at pc=0x%08x", env->pc);
        s1c33_enter_halt(env, cs, true, false);
    }
    ...
    qemu_log_mask(CPU_LOG_INT,
                  "s1c33: RETD debug return pc=0x%08x r0=0x%08x\n",
                  env->pc, env->regs[0]);
    cpu_loop_exit(cs);
}
                                           ...
                                           uint32_t insn)
{
    ...
    warn_report("S1C33 guest shutdown: unimplemented opcode 0x%04x "
                "at pc=0x%08x",
                insn & 0xffff, env->pc);
    qemu_log_mask(LOG_GUEST_ERROR,
