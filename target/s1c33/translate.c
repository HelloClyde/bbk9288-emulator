...
static void gen_software_interrupt(DisasContext *ctx, uint16_t word)
{
    uint32_t vector_offset = 48 + (word & 3) * 4;
    TCGv_i32 old_psr = tcg_temp_new_i32();
    TCGv_i32 vector_addr = tcg_temp_new_i32();
    TCGv_i32 target = tcg_temp_new_i32();

    tcg_gen_mov_i32(old_psr, cpu_psr);
    tcg_gen_subi_i32(cpu_sp, cpu_sp, 4);
    tcg_gen_qemu_st_i32(tcg_constant_i32(ctx->base.pc_next),
                        cpu_sp, 0, MO_LEUL);
    tcg_gen_subi_i32(cpu_sp, cpu_sp, 4);
    tcg_gen_qemu_st_i32(old_psr, cpu_sp, 0, MO_LEUL);
    tcg_gen_andi_i32(vector_addr, cpu_ttbr, S1C33_TTBR_MASK);
    tcg_gen_addi_i32(vector_addr, vector_addr, vector_offset);
    tcg_gen_qemu_ld_i32(target, vector_addr, 0, MO_LEUL);
    tcg_gen_ori_i32(cpu_psr, cpu_psr, S1C33_PSR_IE);
    gen_clear_ext(ctx);
    tcg_gen_mov_i32(cpu_pc, target);
    tcg_gen_lookup_and_goto_ptr();
    ctx->base.is_jmp = DISAS_NORETURN;
...
static void gen_retd(DisasContext *ctx)
{
    tcg_gen_movi_i32(cpu_pc, ctx->pc);
    gen_clear_ext(ctx);
    gen_helper_s1c33_retd(tcg_env);
    ctx->base.is_jmp = DISAS_NORETURN;
}
...
static void gen_mac(DisasContext *ctx, uint16_t word)
{
    gen_helper_s1c33_mac(tcg_env,
                         tcg_constant_i32(extract32(word, 4, 4)));
    gen_clear_ext(ctx);
}

static void gen_mirror(DisasContext *ctx, uint16_t word)
{
    unsigned rs = extract32(word, 4, 4);
    unsigned rd = extract32(word, 0, 4);
    TCGv_i32 value = tcg_temp_new_i32();
    ...
    tcg_gen_mov_i32(value, cpu_regs[rs]);
    tcg_gen_shri_i32(lhs, value, 1);
    tcg_gen_andi_i32(lhs, lhs, 0x55555555);
    tcg_gen_andi_i32(rhs, value, 0x55555555);
    tcg_gen_shli_i32(rhs, rhs, 1);
    tcg_gen_or_i32(value, lhs, rhs);

    tcg_gen_shri_i32(lhs, value, 2);
    tcg_gen_andi_i32(lhs, lhs, 0x33333333);
    tcg_gen_andi_i32(rhs, value, 0x33333333);
    tcg_gen_shli_i32(rhs, rhs, 2);
    tcg_gen_or_i32(value, lhs, rhs);

    tcg_gen_shri_i32(lhs, value, 4);
    tcg_gen_andi_i32(lhs, lhs, 0x0f0f0f0f);
    tcg_gen_andi_i32(rhs, value, 0x0f0f0f0f);
    tcg_gen_shli_i32(rhs, rhs, 4);
    tcg_gen_or_i32(cpu_regs[rd], lhs, rhs);
    gen_clear_ext(ctx);
}
...
static void translate_one(DisasContext *ctx, uint16_t word)
{
    ...
    if (word >= 0x0480 && word <= 0x0483) {
        gen_software_interrupt(ctx, word);
        return;
    }
    ...
    if (word >= 0xb200 && word <= 0xb2f0 && (word & 0xf) == 0) {
        gen_mac(ctx, word);
        return;
    }
...
void s1c33_translate_init(void)
{
    ...
    cpu_ext_count = tcg_global_mem_new_i32(tcg_env,
                                           ...
                                           "ext_count");
    cpu_ttbr = tcg_global_mem_new_i32(tcg_env,
                                      offsetof(CPUS1C33State, ttbr),
                                      "ttbr");
}
