...
- S1C33L05 internal VRAM mapped at `0x003c0000..0x003c9fff` as real 40 KiB
  RAM.
- QEMU graphical console rendering internal VRAM as `160x240`, with
  register-selected 1/2/4/8 bpp unpacking and the LCDC green grayscale LUT.
  The stock firmware selects 2 bpp; 4 bpp exposes the panel's 16 gray levels.
...
- HSDMA channels implement dual- and single-address operation, single,
  continuous, and block transfer modes, source/destination address controls,
  transfer-count decrement/reload, software trigger clearing, channel disable,
  and completion factor update. This includes the V1.5 channel-2 continuous
  512-byte NAND transfer path as well as the earlier channel-1 path.
...
- Board audio decoder bus mapped at `0x003a0000`. The firmware reads data at
  ...
  the stream select is active. The optional `audio-player` property launches an
  ffplay-compatible host process and feeds that same compressed decoder stream
  over stdin for real speaker output. An 8 KiB effective decoder input queue
  drives the real DREQ signal on P0.3 in 32-byte grants; parallel bus
  write-ready at `+0x0a bit 0x10` remains a separate signal. The queue drains
...
- Board-level Samsung-compatible NAND at `0x04000000`, identified as
  ...
  are modeled. The optional `nand-image` machine property opens a persistent
  276,824,064-byte raw image including OOB. Program and erase operations mark
  individual physical blocks dirty; a real-time 250 ms timer writes and
  `fdatasync`s only those blocks. Exit performs a final flush, while completed
  guest writes also survive an abnormal QEMU termination after the periodic
  flush.
...
- QEMU keyboard input mapped to the six firmware-observed hardware lines:
  ...
  paths. This is the 9288S input path.
- The `bbk9288` machine instead models its 8-row by 7-column keyboard matrix.
  Firmware writes an active-low row mask to board latch `0x00300f46`; the four
  low columns are read from `K5D[3:0]` and the remaining three from
  `P0D[6:4]`. Any pressed matrix key also asserts the common `K6.4/FPT4`
  wake line.
- V1.5 translates raw matrix IDs with the 64-byte table at `0x021994dc`.
  The model inverts that observed table against QEMU's Linux input key codes,
  preserving letters, digits, Enter, Delete, PageUp/PageDown, function keys,
  and the four cursor directions. The Windows companion panel exposes the
  resulting 53-key layout over QMP.
- The four 8-bit timer channels use their documented per-channel clock-source
  and prescaler selections, RUN/PRESET behavior, counter reload and underflow
  handling, and consecutive vectors `52..55`. Keyboard debounce runs on
  Timer3/vector `55`; routing all four channels to vector `53` prevents the
  9288 matrix handler from ever running.
...
- C33 `swap` and `swaph`, required by the firmware MP3 frame-header parser,
  plus `int`, `retd`, `brk`, per-byte `mirror`, and signed repeated `mac`.
...
The emulator is not a complete model of every S1C33L05 peripheral, but the
...
confirm, and exit controls, and plays firmware-supplied MP3 data through either
the browser frontend or a local ffplay process.
...
$env:MSYSTEM = 'UCRT64'
& 'C:\msys64\usr\bin\bash.exe' -lc 'mkdir -p _build && cd _build && ../configure --target-list=s1c33-softmmu --disable-werror --disable-docs --disable-tools --enable-sdl --disable-relocatable && ninja qemu-system-s1c33.exe'
```
...
`160x240` framebuffer. `BPP[2:0]` in LCDC MODE0 (`0x00380200`) selects
1/2/4/8 bpp unpacking, and entries at `0x00380400..0x003807ff` provide the
green grayscale LUT. The stock firmware writes `BPP=001` for 2 bpp, while
`BPP=010` selects 4 bpp / 16-gray output.

```powershell
$env:PATH='C:\msys64\ucrt64\bin;C:\msys64\usr\bin;' + $env:PATH
.\_build\qemu-system-s1c33.exe -M bbk9288s,nand-image=runtime/nand-user.raw -display none -serial none -monitor none -d guest_errors,int -D .\_build\bbk9288s-test\display.log
```

To dump the current LCD framebuffer to PGM without a GUI backend:

```powershell
& 'C:\msys64\usr\bin\bash.exe' -lc 'export PATH=/ucrt64/bin:/usr/bin:$PATH; mkdir -p _build/bbk9288s-test; rm -f _build/bbk9288s-test/lcd-dump.pgm _build/bbk9288s-test/run-lcd-dump.log; timeout 3s _build/qemu-system-s1c33.exe -M bbk9288s,nand-image=runtime/nand-user.raw,debug-lcd-dump-ms=1000,debug-lcd-dump-path=_build/bbk9288s-test/lcd-dump.pgm -cpu c33l05,exit-on-halt=off,trace-calls=on -nographic -serial none -monitor none -d guest_errors,int -D _build/bbk9288s-test/run-lcd-dump.log'
```
...
```powershell
python scripts\bbk9288s_qmp_touch_trace.py --prefix nand-format --no-touch --no-default-snapshots --pre-ms 0 --post-ms 55000 --no-trace-key-scan --machine-opt nand-image=_build/bbk9288s-test/nand-formatted.raw --qemu-log-items guest_errors
```

With `pyfatfs` installed, populate a formatted image from the downloaded tree:

```powershell
python scripts\bbk9288s_nand_image.py install _build\bbk9288s-test\nand-formatted.raw C:\path\to\system-tree --output _build\bbk9288s-test\nand-system.raw --flat _build\bbk9288s-test\nand-system.fat.img
```

Run the machine with the resulting persistent image:

```powershell
_build\qemu-system-s1c33.exe -M bbk9288s,nand-image=_build/bbk9288s-test/nand-system.raw -cpu c33l05,exit-on-halt=off -rtc base=localtime -nographic -serial none -monitor none
```
...
$env:CHERE_INVOKING = '1'
$env:MSYSTEM = 'UCRT64'
& 'C:\msys64\usr\bin\bash.exe' -lc 'timeout 5s _build/qemu-system-s1c33.exe -M bbk9288s,nand-image=runtime/nand-user.raw -cpu c33l05,exit-on-halt=off,trace-calls=on -nographic -serial none -monitor none -d guest_errors,int -D _build/bbk9288s-test/run-timerwake.log; echo exit:$?'
```
...
$env:CHERE_INVOKING = '1'
$env:MSYSTEM = 'UCRT64'
& 'C:\msys64\usr\bin\bash.exe' -lc 'timeout 6s _build/qemu-system-s1c33.exe -M bbk9288s,nand-image=runtime/nand-user.raw,debug-port4-wakeup-ms=1000 -cpu c33l05,exit-on-halt=off,trace-calls=on -nographic -serial none -monitor none -d guest_errors,int -D _build/bbk9288s-test/run-port4wake-gpio.log; echo exit:$?'
```
...
$env:PATH='C:\msys64\ucrt64\bin;C:\msys64\usr\bin;' + $env:PATH
$port = 4561
.\_build\qemu-system-s1c33.exe -M bbk9288s,nand-image=runtime/nand-user.raw -cpu c33l05,exit-on-halt=off,trace-calls=on -nographic -serial none -monitor none -qmp tcp:127.0.0.1:$port,server=on,wait=off -d guest_errors,int -D .\_build\bbk9288s-test\run-qmp-key-a-clean.log
```
...
```powershell
& 'C:\msys64\usr\bin\bash.exe' -lc 'export PATH=/ucrt64/bin:/usr/bin:$PATH; timeout 3s _build/qemu-system-s1c33.exe -M bbk9288s,nand-image=runtime/nand-user.raw,trace-key-scan=on,debug-key-p0-low-mask=0x08 -cpu c33l05,exit-on-halt=off,trace-calls=on -nographic -serial none -monitor none -d guest_errors,int -D _build/bbk9288s-test/run-keyscan-debugmask.log'
```
...
$env:CHERE_INVOKING = '1'
$env:MSYSTEM = 'UCRT64'
& 'C:\msys64\usr\bin\bash.exe' -lc 'timeout 2s _build/qemu-system-s1c33.exe -M bbk9288s,nand-image=runtime/nand-user.raw,debug-usb-wakeup-ms=50 -cpu c33l05,exit-on-halt=off -nographic -serial none -monitor none -d guest_errors,int -D _build/bbk9288s-test/wakeup-real.log'
```
...
```powershell
$env:PATH='C:\msys64\ucrt64\bin;C:\msys64\usr\bin;' + $env:PATH
.\_build\qemu-system-s1c33.exe -M bbk9288s,nand-image=runtime/nand-user.raw -cpu c33l05,trace-psr=on -nographic -serial none -monitor none -d guest_errors,int -D .\_build\bbk9288s-test\psr-trace.log
```
...
- The 9288S monochrome panel supports 16 gray levels. The stock firmware
  programs LCDC MODE0 to `BPP=001`, so its normal UI uses 2 bpp / four gray
  levels; `BPP=010` enables the panel's 4 bpp / 16-gray mode.
- Portrait 160x240 framebuffers use 9600 bytes at 2 bpp and 19200 bytes at
  4 bpp, so both modes fit in the 40 KiB internal VRAM.
