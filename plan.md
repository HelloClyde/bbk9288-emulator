...
- 片内 RAM 16 KiB。
- 片内 VRAM 40 KiB。
- 9288S 单色屏支持 16 灰阶；stock 固件默认选择 `2 bpp / 4-level gray scale`。
- 当前按竖屏 `160x240` 建模，LCDC MODE0 可切换 1/2/4/8 bpp；2 bpp
  framebuffer 为 `9600` 字节，4 bpp / 16 灰阶为 `19200` 字节，均可放入片内 VRAM。
...
- 片内 VRAM 按 `160x240` 和 MODE0 的 BPP 位动态展开 1/2/4/8 bpp；
  4 bpp 模式按每字节高、低 nibble 输出两个像素，可显示 16 灰阶。
- 2 bpp byte layout 为一个 byte 内 4 个像素按 bit7..bit0 的 2-bit pair 展开。
- 使用 `0x380400..0x3807ff` 的绿色 LUT 进行灰阶映射。
- direct KNL load 下种 `HSIZE=0x0013`、`VSIZE=0x00ef`、stock `BPP=001`
  以及标准四级灰阶 LUT。
...
- QEMU 使用内置 VNC 服务在 `127.0.0.1:5900` 提供显示，并在
  ...
  硬件模型。
- `web` 使用 noVNC 1.7、Vite 5 和 Lucide，提供 160x240、最高 16 灰阶显示、
...
- QEMU 新增板级音频解码器总线模型和 `audio-stream` 机器属性。模型只在
  ...
  函数，也没有系统或应用 hook。
- 解码器输入端建模为 8 KiB 有效队列，仅剩余空间足够容纳固件下一次
  32 字节传输时才在 P0.3 拉高 DREQ；`0x003a000a bit 0x10` 只表示并行
  总线写口就绪，两条硬件握手信号不再混用。队列按检测到的 MPEG 码率和
  实时时钟消耗，避免固件无限灌流并抢占应用执行。硬件输出达到 32 MiB
  后轮转，限制运行目录增长。
- 补齐 Timer4-B 比较中断（FIR4 bit 2、vector 46）后，固件会执行音频
  周期管理和 SCI 状态查询。控制片选下实现 `0x02` 写、`0x03` 读协议，
  支持 `SCI_MODE` 软件复位、`SCI_CLOCKF`、`SCI_DECODE_TIME`、
  `SCI_AUDATA` 和 `SCI_VOL`；`DECODE_TIME` 按已解码的完整秒数回读，
  固件写入该寄存器时同步重定位解码时钟，停止后重播会从 `00:00` 开始。
- Web 服务从 QEMU 的硬件输出偏移持续等待新码流；浏览器按用户手势建立
  MediaSource，顺序追加 MP3、裁剪旧缓冲并在文件轮转后从零续流，提供
  扬声器静音按钮和音量滑杆，不会重复旧片段。
- “娱乐 -> MP3 -> 步步高.mp3”实测总时长显示 `04:15`；同一浏览器调用
  精确等待 `5.086 s`，当前时间从 `00:38` 递增到 `00:43`。五秒硬件输出
  增长 `80,192` 字节（`15,971.5 B/s`），与文件的 `128 kbps` 基本一致；
  暂停后三秒保持 `01:47` 且输出增量为零，恢复后继续递增。停止并重播的
  独立回归中，五秒增长 `83,111` 字节，播放时间从零重新计数。
- 背光超时和任务栏背光图标实测均通过 P2.3 控制：`P2D` 从 `0x47`
  变为 `0x4f` 时关闭背光，唤醒后恢复为 `0x47`。QEMU 将 P2.3 建模为
  低有效背光使能；关闭背光时保留可反射观看的暗色四灰阶画面，不修改
  LCD VRAM，固件继续负责空闲计时和背光开关策略。
...
- 固件函数 `0x02067826` 读取板级寄存器 `0x01000022` 并返回 bit 7
  (`0x80`)；电池状态函数 `0x020678c8` 在该位有效时跳过电池 ADC 分级，
  进入外部供电状态。
- 2026-07-29 空闲实测中，未连接实例约五分钟后由 guest 正常关机；运行时
  将 bit 7 置位后，同一实例越过该关机点并持续运行，确认 USB/VBUS 会抑制
  固件的空闲自动关机路径。
- QEMU `usb-connected` 机器属性默认 `on`，直接驱动上述板级寄存器位。
- Web `设置` 页集中提供 USB 开关与按键映射。USB 状态持久化到运行目录的
  `settings.json`，修改时正常重启 QEMU；默认按键仍为
  `W/A/S/D`、`J`、`K`。

## Web 快速触摸队列

- 旧前端会把短触摸延长到 180 ms，但释放定时器完成前
  `activeTouchPointer` 一直非空，期间的新 `pointerdown` 会被直接丢弃。
- Web 端现将后续触摸保存为最多 12 项的 FIFO；每项仍发送独立的 RFB
  `pen down/up`，保持 180 ms 按下和 60 ms 松开窗口，不修改 guest 内存。
- 浏览器快速双击回归中，QEMU 日志稳定收到
  `down/up/down/up` 四个板级触摸事件；切换页面、失焦、重连和重启会清空
  队列并显式释放当前触摸，避免粘住 FPT6。
